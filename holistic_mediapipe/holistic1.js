...
let showTesselation = true;
function cameraCapture(supportedDevices) {
...
const config = {
    locateFile: (file) => {
        // console.log(`https://cdn.jsdelivr.net/npm/@mediapipe/holistic@` + `${mpHolistic.VERSION}/${file}`)
...
function onResults(results) {
    ...
    if (showTesselation !== false){
        drawingUtils.drawConnectors(canvasCtx, results.faceLandmarks, mpHolistic.FACEMESH_TESSELATION, {color: 'rgb(194, 254, 15)', lineWidth: 1});
    }
    ...
    drawingUtils.drawConnectors(canvasCtx, results.faceLandmarks, mpHolistic.FACEMESH_LIPS, { color: 'rgb(231, 35, 0)', lineWidth: 5 });
    canvasCtx.restore();
}
